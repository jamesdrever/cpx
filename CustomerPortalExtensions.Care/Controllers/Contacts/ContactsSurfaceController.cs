...
using Membership = CustomerPortalExtensions.Domain.Membership.Membership;
...
namespace CustomerPortalExtensions.Controllers.Contacts
{
    ...
    public class ContactsSurfaceController : CpxBaseSurfaceController
    {
        ...
        public ContactsSurfaceController(IContactSynchroniser contactSynchroniser,IContactService contactService)
        {
            ...
            this._contactSynchroniser = contactSynchroniser;
        ...
        public ActionResult SaveContact([Bind(Prefix = "Contact")]ContactViewModel contact)
        {
            if (ModelState.IsValid)
            {
                string url = Request["Contact.url"] == null ? "" : Request["Contact.url"];
                Contact updateContact = (Contact) new Contact().InjectFrom(contact);
        ...
        [HttpPost]
        public ActionResult SavePreferences(ContactPreferencesViewModel preferences, string[] selectedPreferences, string [] allPreferences)
        {
            if (ModelState.IsValid)
            {
                ...
                //{
                    ...
                    {
                        ContactPreferences preferencesToUpdate = getContactPreferencesToUpdate(preferences,selectedPreferences,allPreferences);
                        ContactPreferencesOperationStatus savePreferenceStatus=_contactSynchroniser.SaveContactPreferences(preferencesToUpdate);
        ...
        private ContactPreferences getContactPreferencesToUpdate(ContactPreferencesViewModel preferences, string[] selectedPreferences, string[] allPreferences)
        {
            ...
            foreach (string preference in selectedPreferences)
            {
                ...
                listofSelectedPreferences.Add(new Preference { Category = splitPreferences[0], Value = splitPreferences[1] });
            }
            ...
            foreach (string preference in allPreferences)
            {
                ...
                bool selected = listofSelectedPreferences.Any(s => s.Category.Equals(preferenceCategory) && s.Value.Equals(preferenceValue));
                preferencesToUpdate.Preferences.Add(new Preference { Category = preferenceCategory, Value = preferenceValue, Selected = selected });
            }
        ...
        public ActionResult SaveMembership(MembershipViewModel membership)
        {
            if (ModelState.IsValid)
            {
                Membership createMembership = new Membership();
        ...
        {
            List<MembershipType> listOfMembershipTypes = new List<MembershipType>();
            foreach (var membershipType in (CurrentPage.Children))
            {
                //TODO: this - doc type alias- shouldn't be hard-coded
                if (membershipType.DocumentTypeAlias == "FSCMembership")
                {
                    listOfMembershipTypes.Add(new MembershipType() { Description = membershipType.Name,Code = membershipType.Id.ToNullSafeString(), Price = Convert.ToDecimal(membershipType.GetProperty("Price").Value) });
                }
            }
